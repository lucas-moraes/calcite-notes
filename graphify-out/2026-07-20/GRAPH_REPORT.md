# Graph Report - .  (2026-06-15)

## Corpus Check
- 108 files · ~201,379 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 946 nodes · 1317 edges · 157 communities (100 shown, 57 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 38 edges (avg confidence: 0.83)
- Token cost: 0 input · 0 output

## Community Hubs (Navigation)
- [[_COMMUNITY_ACL Permission Schema|ACL Permission Schema]]
- [[_COMMUNITY_NPM Dependencies|NPM Dependencies]]
- [[_COMMUNITY_Core Command Types|Core Command Types]]
- [[_COMMUNITY_Event Permission Schema|Event Permission Schema]]
- [[_COMMUNITY_Git Operations|Git Operations]]
- [[_COMMUNITY_App Configuration|App Configuration]]
- [[_COMMUNITY_Window Permission Schema|Window Permission Schema]]
- [[_COMMUNITY_Menu Permission Schema|Menu Permission Schema]]
- [[_COMMUNITY_App Icons & Assets|App Icons & Assets]]
- [[_COMMUNITY_Tauri App Config|Tauri App Config]]
- [[_COMMUNITY_Core ACL Manifests|Core ACL Manifests]]
- [[_COMMUNITY_Filesystem Operations|Filesystem Operations]]
- [[_COMMUNITY_TypeScript Config|TypeScript Config]]
- [[_COMMUNITY_UI Components|UI Components]]
- [[_COMMUNITY_Editor & Architecture|Editor & Architecture]]
- [[_COMMUNITY_Git UI & API Bridge|Git UI & API Bridge]]
- [[_COMMUNITY_Desktop Permission Schema|Desktop Permission Schema]]
- [[_COMMUNITY_Theming & Branding|Theming & Branding]]
- [[_COMMUNITY_macOS Permission Schema|macOS Permission Schema]]
- [[_COMMUNITY_Tauri App Setup|Tauri App Setup]]
- [[_COMMUNITY_Module Group 20|Module Group 20]]
- [[_COMMUNITY_Module Group 21|Module Group 21]]
- [[_COMMUNITY_Module Group 22|Module Group 22]]
- [[_COMMUNITY_Module Group 23|Module Group 23]]
- [[_COMMUNITY_Module Group 24|Module Group 24]]
- [[_COMMUNITY_Module Group 25|Module Group 25]]
- [[_COMMUNITY_Module Group 26|Module Group 26]]
- [[_COMMUNITY_Module Group 27|Module Group 27]]
- [[_COMMUNITY_Module Group 28|Module Group 28]]
- [[_COMMUNITY_Module Group 29|Module Group 29]]
- [[_COMMUNITY_Module Group 30|Module Group 30]]
- [[_COMMUNITY_Module Group 31|Module Group 31]]
- [[_COMMUNITY_Module Group 32|Module Group 32]]
- [[_COMMUNITY_Module Group 33|Module Group 33]]
- [[_COMMUNITY_Module Group 34|Module Group 34]]
- [[_COMMUNITY_Module Group 35|Module Group 35]]
- [[_COMMUNITY_Module Group 36|Module Group 36]]
- [[_COMMUNITY_Module Group 37|Module Group 37]]
- [[_COMMUNITY_Module Group 38|Module Group 38]]
- [[_COMMUNITY_Module Group 39|Module Group 39]]
- [[_COMMUNITY_Module Group 40|Module Group 40]]
- [[_COMMUNITY_Module Group 41|Module Group 41]]
- [[_COMMUNITY_Module Group 42|Module Group 42]]
- [[_COMMUNITY_Module Group 43|Module Group 43]]
- [[_COMMUNITY_Module Group 44|Module Group 44]]
- [[_COMMUNITY_Module Group 45|Module Group 45]]
- [[_COMMUNITY_Module Group 46|Module Group 46]]
- [[_COMMUNITY_Module Group 47|Module Group 47]]
- [[_COMMUNITY_Module Group 48|Module Group 48]]
- [[_COMMUNITY_Module Group 49|Module Group 49]]
- [[_COMMUNITY_Module Group 50|Module Group 50]]
- [[_COMMUNITY_Module Group 51|Module Group 51]]
- [[_COMMUNITY_Module Group 52|Module Group 52]]
- [[_COMMUNITY_Module Group 53|Module Group 53]]
- [[_COMMUNITY_Module Group 54|Module Group 54]]
- [[_COMMUNITY_Module Group 55|Module Group 55]]
- [[_COMMUNITY_Module Group 56|Module Group 56]]
- [[_COMMUNITY_Module Group 57|Module Group 57]]
- [[_COMMUNITY_Module Group 58|Module Group 58]]
- [[_COMMUNITY_Module Group 59|Module Group 59]]
- [[_COMMUNITY_Module Group 60|Module Group 60]]
- [[_COMMUNITY_Module Group 61|Module Group 61]]
- [[_COMMUNITY_Module Group 62|Module Group 62]]
- [[_COMMUNITY_Module Group 63|Module Group 63]]
- [[_COMMUNITY_Module Group 64|Module Group 64]]
- [[_COMMUNITY_Module Group 65|Module Group 65]]
- [[_COMMUNITY_Module Group 66|Module Group 66]]
- [[_COMMUNITY_Module Group 67|Module Group 67]]
- [[_COMMUNITY_Module Group 68|Module Group 68]]
- [[_COMMUNITY_Module Group 69|Module Group 69]]
- [[_COMMUNITY_Module Group 70|Module Group 70]]
- [[_COMMUNITY_Module Group 71|Module Group 71]]
- [[_COMMUNITY_Module Group 72|Module Group 72]]
- [[_COMMUNITY_Module Group 73|Module Group 73]]
- [[_COMMUNITY_Module Group 74|Module Group 74]]
- [[_COMMUNITY_Module Group 75|Module Group 75]]
- [[_COMMUNITY_Module Group 76|Module Group 76]]
- [[_COMMUNITY_Module Group 77|Module Group 77]]
- [[_COMMUNITY_Module Group 78|Module Group 78]]
- [[_COMMUNITY_Module Group 79|Module Group 79]]
- [[_COMMUNITY_Module Group 80|Module Group 80]]
- [[_COMMUNITY_Module Group 81|Module Group 81]]
- [[_COMMUNITY_Module Group 82|Module Group 82]]
- [[_COMMUNITY_Module Group 83|Module Group 83]]
- [[_COMMUNITY_Module Group 84|Module Group 84]]
- [[_COMMUNITY_Module Group 85|Module Group 85]]
- [[_COMMUNITY_Module Group 86|Module Group 86]]
- [[_COMMUNITY_Module Group 87|Module Group 87]]
- [[_COMMUNITY_Module Group 88|Module Group 88]]
- [[_COMMUNITY_Module Group 89|Module Group 89]]
- [[_COMMUNITY_Module Group 90|Module Group 90]]
- [[_COMMUNITY_Module Group 91|Module Group 91]]
- [[_COMMUNITY_Module Group 92|Module Group 92]]
- [[_COMMUNITY_Module Group 93|Module Group 93]]
- [[_COMMUNITY_Module Group 94|Module Group 94]]
- [[_COMMUNITY_Module Group 95|Module Group 95]]
- [[_COMMUNITY_Module Group 96|Module Group 96]]
- [[_COMMUNITY_Module Group 97|Module Group 97]]
- [[_COMMUNITY_Module Group 98|Module Group 98]]
- [[_COMMUNITY_Module Group 99|Module Group 99]]
- [[_COMMUNITY_Module Group 100|Module Group 100]]
- [[_COMMUNITY_Module Group 101|Module Group 101]]
- [[_COMMUNITY_Module Group 103|Module Group 103]]
- [[_COMMUNITY_Module Group 106|Module Group 106]]
- [[_COMMUNITY_Module Group 107|Module Group 107]]
- [[_COMMUNITY_Module Group 108|Module Group 108]]
- [[_COMMUNITY_Module Group 109|Module Group 109]]
- [[_COMMUNITY_Module Group 110|Module Group 110]]
- [[_COMMUNITY_Module Group 111|Module Group 111]]
- [[_COMMUNITY_Module Group 112|Module Group 112]]
- [[_COMMUNITY_Module Group 113|Module Group 113]]
- [[_COMMUNITY_Module Group 114|Module Group 114]]
- [[_COMMUNITY_Module Group 115|Module Group 115]]
- [[_COMMUNITY_Module Group 116|Module Group 116]]
- [[_COMMUNITY_Module Group 117|Module Group 117]]
- [[_COMMUNITY_Module Group 118|Module Group 118]]
- [[_COMMUNITY_Module Group 119|Module Group 119]]
- [[_COMMUNITY_Module Group 120|Module Group 120]]
- [[_COMMUNITY_Module Group 121|Module Group 121]]
- [[_COMMUNITY_Module Group 122|Module Group 122]]
- [[_COMMUNITY_Module Group 123|Module Group 123]]
- [[_COMMUNITY_Module Group 124|Module Group 124]]
- [[_COMMUNITY_Module Group 125|Module Group 125]]
- [[_COMMUNITY_Module Group 126|Module Group 126]]
- [[_COMMUNITY_Module Group 127|Module Group 127]]
- [[_COMMUNITY_Module Group 128|Module Group 128]]
- [[_COMMUNITY_Module Group 129|Module Group 129]]
- [[_COMMUNITY_Module Group 130|Module Group 130]]
- [[_COMMUNITY_Module Group 131|Module Group 131]]
- [[_COMMUNITY_Module Group 132|Module Group 132]]
- [[_COMMUNITY_Module Group 133|Module Group 133]]
- [[_COMMUNITY_Module Group 134|Module Group 134]]
- [[_COMMUNITY_Module Group 135|Module Group 135]]
- [[_COMMUNITY_Module Group 136|Module Group 136]]
- [[_COMMUNITY_Module Group 137|Module Group 137]]
- [[_COMMUNITY_Module Group 138|Module Group 138]]
- [[_COMMUNITY_Module Group 139|Module Group 139]]
- [[_COMMUNITY_Module Group 140|Module Group 140]]
- [[_COMMUNITY_Module Group 141|Module Group 141]]
- [[_COMMUNITY_Module Group 142|Module Group 142]]
- [[_COMMUNITY_Module Group 143|Module Group 143]]
- [[_COMMUNITY_Module Group 144|Module Group 144]]
- [[_COMMUNITY_Module Group 145|Module Group 145]]
- [[_COMMUNITY_Module Group 146|Module Group 146]]
- [[_COMMUNITY_Module Group 147|Module Group 147]]
- [[_COMMUNITY_Module Group 148|Module Group 148]]
- [[_COMMUNITY_Module Group 149|Module Group 149]]
- [[_COMMUNITY_Module Group 150|Module Group 150]]
- [[_COMMUNITY_Module Group 151|Module Group 151]]
- [[_COMMUNITY_Module Group 152|Module Group 152]]
- [[_COMMUNITY_Module Group 153|Module Group 153]]
- [[_COMMUNITY_Module Group 154|Module Group 154]]

## God Nodes (most connected - your core abstractions)
1. `allow` - 76 edges
2. `deny` - 76 edges
3. `permissions` - 31 edges
4. `permissions` - 30 edges
5. `App()` - 19 edges
6. `Calcite Notes App Icon` - 19 edges
7. `AppHandle` - 15 edges
8. `Note` - 15 edges
9. `compilerOptions` - 15 edges
10. `AppState` - 12 edges

## Surprising Connections (you probably didn't know these)
- `KeyboardShortcuts` --conceptually_related_to--> `Plugin System Design`  [INFERRED]
  src/components/WysiwygEditor.tsx → FEAT-PLUGINS-IMPLEMENTATION.md
- `doSearch()` --references--> `search_notes()`  [INFERRED]
  src/components/CommandPalette.tsx → src-tauri/src/commands/notes.rs
- `App()` --references--> `set_notes_dir()`  [INFERRED]
  src/App.tsx → src-tauri/src/commands/config.rs
- `App()` --references--> `get_theme()`  [INFERRED]
  src/App.tsx → src-tauri/src/commands/config.rs
- `App()` --references--> `save_theme()`  [INFERRED]
  src/App.tsx → src-tauri/src/commands/config.rs

## Import Cycles
- 1-file cycle: `src-tauri/src/commands/config.rs -> src-tauri/src/commands/config.rs`
- 1-file cycle: `src-tauri/src/commands/filesystem.rs -> src-tauri/src/commands/filesystem.rs`
- 1-file cycle: `src-tauri/src/commands/git.rs -> src-tauri/src/commands/git.rs`
- 1-file cycle: `src-tauri/src/commands/mod.rs -> src-tauri/src/commands/mod.rs`
- 1-file cycle: `src-tauri/src/commands/notes.rs -> src-tauri/src/commands/notes.rs`
- 1-file cycle: `src-tauri/src/lib.rs -> src-tauri/src/lib.rs`

## Hyperedges (group relationships)
- **Tauri Platform Capability Schemas** — concept_tauri_capability, schemas_desktop_schema_capabilityfile, schemas_macos_schema_capabilityfile [EXTRACTED 1.00]
- **Frontend components calling tauriAPI for filesystem and config operations** — commands_filetree, src_lib_tauri, commands_sidebar, commands_graphview [INFERRED 0.75]
- **Theme System: definition → application → persistence** — src_lib_themes_themes, src_lib_themes_setthemecolors, src_lib_tauri_tauriapi [INFERRED 0.85]
- **App Icon Multi-Resolution Variants** — concept_app_icon, icons_128x128_icon, icons_128x128_2x_icon, icons_32x32_icon, icons_64x64_icon, icons_square107_icon, icons_square142_icon, icons_square150_icon, icons_square284_icon, icons_square30_icon, icons_square310_icon, resources_icons_128_icon, resources_icons_16_icon, resources_icons_256_icon, resources_icons_32_icon, resources_icons_48_icon, resources_icons_512_icon, resources_icons_64_icon, assets_512x512_removebg_icon [EXTRACTED 1.00]
- **Tauri IPC Command Registration** — commands_notes_get_notes, commands_notes_get_all_notes_for_graph, commands_notes_save_note, commands_notes_delete_note, commands_notes_save_new_note, commands_notes_rename_note, commands_notes_update_note_tags, commands_notes_update_note_properties, commands_notes_search_notes, commands_filesystem_delete_folder, commands_filesystem_create_folder, commands_filesystem_rename_folder, commands_filesystem_move_file, commands_filesystem_create_file, commands_filesystem_get_directory, commands_filesystem_read_file, commands_config_get_notes_folder, commands_config_set_notes_dir, commands_config_get_theme, commands_config_save_theme, commands_git_git_init, commands_git_git_status, commands_git_git_commit, commands_git_git_log, commands_git_git_diff_file, commands_git_git_restore_file [EXTRACTED 1.00]
- **Git Version Control Flow** — commands_git_git_init, commands_git_git_status, commands_git_git_commit, commands_git_git_log, commands_git_git_diff_file, commands_git_git_restore_file, commands_git_get_repo, commands_git_gitcommit, commands_git_gitfilestatus, commands_git_gitfilediff [EXTRACTED 1.00]
- **Path Sandboxing Security Pattern** — commands_notes_is_path_within_notes_dir, commands_filesystem_is_path_within_notes_dir, commands_notes_save_note, commands_notes_delete_note, commands_notes_save_new_note, commands_notes_rename_note, commands_notes_update_note_tags, commands_notes_update_note_properties, commands_filesystem_delete_folder, commands_filesystem_create_folder, commands_filesystem_rename_folder, commands_filesystem_move_file, commands_filesystem_create_file, commands_filesystem_read_file, commands_git_git_diff_file, commands_git_git_restore_file [EXTRACTED 1.00]
- **Note View Components** — components_dataviewpanel, components_graphview, components_tabbar, components_propertiespanel [INFERRED 0.85]
- **Plugin System Design Artifacts** — calcite_plugin_system, calcite_feature_gaps, rationale_plugin_storage_path, rationale_manifest_indexjs, rationale_new_function_eval, rationale_react_lazy_panels, rationale_sha256_checksums [EXTRACTED 0.95]

## Communities (157 total, 57 thin omitted)

### Community 0 - "ACL Permission Schema"
Cohesion: 0.05
Nodes (44): commands, description, identifier, commands, description, identifier, commands, description (+36 more)

### Community 1 - "NPM Dependencies"
Cohesion: 0.05
Nodes (37): dependencies, clsx, d3, lucide-react, marked, motion, prosemirror-markdown, react (+29 more)

### Community 2 - "Core Command Types"
Cohesion: 0.17
Nodes (34): FileEntry, FileReadResult, GraphNote, Note, OperationResult, parse_frontmatter_full(), parse_tags(), strip_frontmatter() (+26 more)

### Community 3 - "Event Permission Schema"
Cohesion: 0.06
Nodes (33): commands, description, identifier, commands, description, identifier, commands, description (+25 more)

### Community 4 - "Git Operations"
Cohesion: 0.20
Nodes (26): get_repo(), git_commit(), git_diff_file(), git_init(), git_log(), git_restore_file(), git_status(), GitCommit (+18 more)

### Community 5 - "App Configuration"
Cohesion: 0.18
Nodes (24): get_active_tab(), get_editor_mode(), get_notes_folder(), get_open_tabs(), get_show_graph(), get_sidebar_width(), get_theme(), get_tree_width() (+16 more)

### Community 6 - "Window Permission Schema"
Cohesion: 0.08
Nodes (25): commands, description, identifier, commands, description, identifier, commands, description (+17 more)

### Community 7 - "Menu Permission Schema"
Cohesion: 0.08
Nodes (25): commands, description, identifier, commands, description, identifier, commands, description (+17 more)

### Community 8 - "App Icons & Assets"
Cohesion: 0.10
Nodes (22): App Icon 512x512 Design (removebg), Calcite Notes App Icon, Tauri Capability System, App Icon 128x128@2x (tauri), App Icon 128x128 (tauri), App Icon 32x32 (tauri), App Icon 64x64 (tauri), App Icon Square107x107Logo (tauri) (+14 more)

### Community 9 - "Tauri App Config"
Cohesion: 0.10
Nodes (20): Default Capability (permissions), app, security, windows, build, beforeBuildCommand, beforeDevCommand, devUrl (+12 more)

### Community 10 - "Core ACL Manifests"
Cohesion: 0.16
Nodes (19): core, core:app, default_permission, global_scope_schema, permission_sets, default_permission, core:event, default_permission (+11 more)

### Community 11 - "Filesystem Operations"
Cohesion: 0.30
Nodes (18): create_file(), create_folder(), delete_folder(), get_directory(), has_md_files(), is_path_within_notes_dir(), move_file(), read_file() (+10 more)

### Community 12 - "TypeScript Config"
Cohesion: 0.11
Nodes (17): compilerOptions, allowImportingTsExtensions, allowJs, experimentalDecorators, isolatedModules, jsx, lib, module (+9 more)

### Community 13 - "UI Components"
Cohesion: 0.16
Nodes (10): WikiLinkPopup, Command, CommandPaletteProps, DataviewPanelProps, SortDir, SortKey, TabBarProps, WikiLinkPopupProps (+2 more)

### Community 14 - "Editor & Architecture"
Cohesion: 0.13
Nodes (13): Calcite Architecture, Feature Gaps vs Obsidian, Plugin System Design, Calcite, KeyboardShortcuts, ToolbarItem, turndown, WysiwygEditorProps (+5 more)

### Community 15 - "Git UI & API Bridge"
Cohesion: 0.22
Nodes (11): GitDiffViewProps, FileEntry, FileReadResult, GitCommit, GitFileDiff, GitFileStatus, GraphNote, OperationResult (+3 more)

### Community 16 - "Desktop Permission Schema"
Cohesion: 0.12
Nodes (16): anyOf, description, definitions, Application, Number, PermissionEntry, ShellScopeEntryAllowedArg, Target (+8 more)

### Community 17 - "Theming & Branding"
Cohesion: 0.20
Nodes (9): setThemeColors(), ThemeColors, ThemeDefinition, ThemeMode, ThemePreset, themes, cn(), formatTime() (+1 more)

### Community 18 - "macOS Permission Schema"
Cohesion: 0.13
Nodes (15): anyOf, description, definitions, Application, Identifier, Number, PermissionEntry, Target (+7 more)

### Community 19 - "Tauri App Setup"
Cohesion: 0.23
Nodes (12): Box, Error, get_default_notes_dir(), run(), select_notes_folder(), setup_menu(), AppHandle, AppState (+4 more)

### Community 20 - "Module Group 20"
Cohesion: 0.24
Nodes (7): Sidebar, GraphViewProps, SidebarProps, GraphLink, GraphNode, GraphLink, GraphNode

### Community 21 - "Module Group 21"
Cohesion: 0.15
Nodes (13): commands, description, identifier, commands, description, identifier, allow, commands (+5 more)

### Community 22 - "Module Group 22"
Cohesion: 0.15
Nodes (13): commands, description, identifier, commands, description, identifier, commands, description (+5 more)

### Community 23 - "Module Group 23"
Cohesion: 0.15
Nodes (13): properties, Identifier, description, oneOf, type, default, description, type (+5 more)

### Community 24 - "Module Group 24"
Cohesion: 0.17
Nodes (12): properties, default, description, type, default, description, type, description (+4 more)

### Community 25 - "Module Group 25"
Cohesion: 0.20
Nodes (10): $ref, description, items, type, uniqueItems, description, items, type (+2 more)

### Community 26 - "Module Group 26"
Cohesion: 0.20
Nodes (10): type, webviews, windows, items, description, items, type, description (+2 more)

### Community 27 - "Module Group 27"
Cohesion: 0.20
Nodes (10): $ref, description, items, type, uniqueItems, description, items, type (+2 more)

### Community 28 - "Module Group 28"
Cohesion: 0.20
Nodes (10): type, webviews, windows, items, description, items, type, description (+2 more)

### Community 29 - "Module Group 29"
Cohesion: 0.25
Nodes (7): ContextMenuState, FileNode, FileTree(), FileTreeProps, formatPathname(), TreeNodeProps, tauriAPI

### Community 30 - "Module Group 30"
Cohesion: 0.22
Nodes (9): devDependencies, autoprefixer, tailwindcss, tailwindcss-animate, @tauri-apps/cli, tsx, @types/node, typescript (+1 more)

### Community 31 - "Module Group 31"
Cohesion: 0.22
Nodes (9): scripts, build, dev, lint, tauri, tauri:build, tauri:dev, vite:build (+1 more)

### Community 32 - "Module Group 32"
Cohesion: 0.25
Nodes (8): description, properties, required, type, CapabilityRemote, urls, description, type

### Community 33 - "Module Group 33"
Cohesion: 0.25
Nodes (8): description, properties, required, type, CapabilityRemote, urls, description, type

### Community 34 - "Module Group 34"
Cohesion: 0.29
Nodes (3): ErrorBoundary, Props, State

### Community 35 - "Module Group 35"
Cohesion: 0.29
Nodes (6): fill, automatic-gradient, groups, supported-platforms, circles, squares

### Community 36 - "Module Group 36"
Cohesion: 0.29
Nodes (6): default, description, identifier, local, permissions, windows

### Community 37 - "Module Group 37"
Cohesion: 0.29
Nodes (6): computedHash, source, sourceType, skills, electron-development, version

### Community 38 - "Module Group 38"
Cohesion: 0.33
Nodes (5): description, identifier, permissions, $schema, windows

### Community 39 - "Module Group 39"
Cohesion: 0.33
Nodes (3): BUILTIN_KEYS, PropertiesPanelProps, PropertyRowProps

### Community 40 - "Module Group 40"
Cohesion: 0.33
Nodes (5): description, majorCapabilities, name, requestFramePermissions, Package Configuration

### Community 41 - "Module Group 41"
Cohesion: 0.40
Nodes (4): description, name, private, version

### Community 42 - "Module Group 42"
Cohesion: 0.40
Nodes (4): anyOf, description, $schema, title

### Community 43 - "Module Group 43"
Cohesion: 0.40
Nodes (4): anyOf, description, $schema, title

### Community 44 - "Module Group 44"
Cohesion: 0.50
Nodes (3): permission, skill, $schema

### Community 45 - "Module Group 45"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-append

### Community 46 - "Module Group 46"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-bundle-type

### Community 47 - "Module Group 47"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-create-default

### Community 48 - "Module Group 48"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-get

### Community 49 - "Module Group 49"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-identifier

### Community 50 - "Module Group 50"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-insert

### Community 51 - "Module Group 51"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-is-checked

### Community 52 - "Module Group 52"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-is-enabled

### Community 53 - "Module Group 53"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-items

### Community 54 - "Module Group 54"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-name

### Community 55 - "Module Group 55"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-prepend

### Community 56 - "Module Group 56"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-register-listener

### Community 57 - "Module Group 57"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-remove-at

### Community 58 - "Module Group 58"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-remove-data-store

### Community 59 - "Module Group 59"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-set-as-windows-menu-for-nsapp

### Community 60 - "Module Group 60"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-set-checked

### Community 61 - "Module Group 61"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-set-dock-visibility

### Community 62 - "Module Group 62"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-set-enabled

### Community 63 - "Module Group 63"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-set-icon

### Community 64 - "Module Group 64"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-set-text

### Community 65 - "Module Group 65"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-tauri-version

### Community 66 - "Module Group 66"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-text

### Community 67 - "Module Group 67"
Cohesion: 0.50
Nodes (4): commands, description, identifier, allow-version

### Community 68 - "Module Group 68"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-app-show

### Community 69 - "Module Group 69"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-append

### Community 70 - "Module Group 70"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-bundle-type

### Community 71 - "Module Group 71"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-default-window-icon

### Community 72 - "Module Group 72"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-fetch-data-store-identifiers

### Community 73 - "Module Group 73"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-insert

### Community 74 - "Module Group 74"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-is-checked

### Community 75 - "Module Group 75"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-is-enabled

### Community 76 - "Module Group 76"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-items

### Community 77 - "Module Group 77"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-name

### Community 78 - "Module Group 78"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-register-listener

### Community 79 - "Module Group 79"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-remove-data-store

### Community 80 - "Module Group 80"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-set-app-theme

### Community 81 - "Module Group 81"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-set-dock-visibility

### Community 82 - "Module Group 82"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-supports-multiple-windows

### Community 83 - "Module Group 83"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-tauri-version

### Community 84 - "Module Group 84"
Cohesion: 0.50
Nodes (4): commands, description, identifier, deny-version

### Community 85 - "Module Group 85"
Cohesion: 0.50
Nodes (4): description, required, type, Capability

### Community 86 - "Module Group 86"
Cohesion: 0.50
Nodes (4): default, description, type, description

### Community 87 - "Module Group 87"
Cohesion: 0.50
Nodes (4): description, required, type, Capability

### Community 88 - "Module Group 88"
Cohesion: 0.50
Nodes (4): ThemeColors, ThemeDefinition, ThemeMode, ThemePreset

### Community 94 - "Module Group 94"
Cohesion: 0.67
Nodes (3): ShellScopeEntryAllowedArgs, anyOf, description

### Community 95 - "Module Group 95"
Cohesion: 0.67
Nodes (3): Value, anyOf, description

### Community 96 - "Module Group 96"
Cohesion: 0.67
Nodes (3): ShellScopeEntryAllowedArg, anyOf, description

### Community 97 - "Module Group 97"
Cohesion: 0.67
Nodes (3): ShellScopeEntryAllowedArgs, anyOf, description

### Community 98 - "Module Group 98"
Cohesion: 0.67
Nodes (3): Value, anyOf, description

### Community 99 - "Module Group 99"
Cohesion: 0.67
Nodes (3): description, type, identifier

## Knowledge Gaps
- **477 isolated node(s):** `$schema`, `plugin`, `@opencode-ai/plugin`, `automatic-gradient`, `groups` (+472 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **57 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `allow` connect `Module Group 21` to `ACL Permission Schema`, `Event Permission Schema`, `Window Permission Schema`, `Menu Permission Schema`, `Module Group 22`, `Module Group 45`, `Module Group 46`, `Module Group 47`, `Module Group 48`, `Module Group 49`, `Module Group 50`, `Module Group 51`, `Module Group 52`, `Module Group 53`, `Module Group 54`, `Module Group 55`, `Module Group 56`, `Module Group 57`, `Module Group 58`, `Module Group 59`, `Module Group 60`, `Module Group 61`, `Module Group 62`, `Module Group 63`, `Module Group 64`, `Module Group 65`, `Module Group 66`, `Module Group 67`, `Module Group 68`, `Module Group 69`, `Module Group 70`, `Module Group 71`, `Module Group 72`, `Module Group 73`, `Module Group 74`, `Module Group 75`, `Module Group 76`, `Module Group 77`, `Module Group 78`, `Module Group 79`, `Module Group 80`, `Module Group 81`, `Module Group 82`, `Module Group 83`, `Module Group 84`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `deny` connect `Module Group 22` to `ACL Permission Schema`, `Event Permission Schema`, `Window Permission Schema`, `Menu Permission Schema`, `Module Group 21`, `Module Group 45`, `Module Group 46`, `Module Group 47`, `Module Group 48`, `Module Group 49`, `Module Group 50`, `Module Group 51`, `Module Group 52`, `Module Group 53`, `Module Group 54`, `Module Group 55`, `Module Group 56`, `Module Group 57`, `Module Group 58`, `Module Group 59`, `Module Group 60`, `Module Group 61`, `Module Group 62`, `Module Group 63`, `Module Group 64`, `Module Group 65`, `Module Group 66`, `Module Group 67`, `Module Group 68`, `Module Group 69`, `Module Group 70`, `Module Group 71`, `Module Group 72`, `Module Group 73`, `Module Group 74`, `Module Group 75`, `Module Group 76`, `Module Group 77`, `Module Group 78`, `Module Group 79`, `Module Group 80`, `Module Group 81`, `Module Group 82`, `Module Group 83`, `Module Group 84`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `App()` connect `Git Operations` to `Theming & Branding`, `Core Command Types`, `Filesystem Operations`, `App Configuration`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **What connects `$schema`, `plugin`, `@opencode-ai/plugin` to the rest of the system?**
  _482 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `ACL Permission Schema` be split into smaller, more focused modules?**
  _Cohesion score 0.045454545454545456 - nodes in this community are weakly interconnected._
- **Should `NPM Dependencies` be split into smaller, more focused modules?**
  _Cohesion score 0.05405405405405406 - nodes in this community are weakly interconnected._
- **Should `Event Permission Schema` be split into smaller, more focused modules?**
  _Cohesion score 0.06060606060606061 - nodes in this community are weakly interconnected._